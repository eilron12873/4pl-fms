<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('New adjustment or write-off') }}</h2></x-slot>
    <div class="py-4 max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
            <form method="POST" action="{{ route('inventory-valuation.adjustments.store') }}">
                @csrf
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="type" name="type" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">
                                ...
                                <option value="write_off" {{ old('type') === 'write_off' ? 'selected' : '' }}>{{ __('Write-off') }}</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">{{ __('Positive for adjustment (add), any for write-off (deducted)') }}</p>
                        </div>
                        <div>
                    ...
                    <div>
                        <label for="reason" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Reason / notes') }}</label>
                        <textarea id="reason" name="reason" rows="2" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">{{ old('reason') }}</textarea>
                    </div>
                </div>
                <div class="mt-6 flex gap-2">
                    ...
                    <a href="{{ route('inventory-valuation.adjustments.index') }}" class="inline-flex px-4 py-2 rounded-md bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
