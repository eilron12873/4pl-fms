<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Edit bill') }} {{ $bill->bill_number }}</h2>
            <a href="{{ route('accounts-payable.bills.show', $bill->id) }}" class="text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-400">{{ __('Back to Bill') }}</a>
        </div>
    </x-slot>
    <div class="py-4 max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
            <form method="POST" action="{{ route('accounts-payable.bills.update', $bill->id) }}">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="vendor_id" name="vendor_id" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">
                            @foreach($vendors as $v)
                                <option value="{{ $v->id }}" {{ old('vendor_id', $bill->vendor_id) == $v->id ? 'selected' : '' }}>{{ $v->code }} - {{ $v->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="currency" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Currency') }}</label>
                        <input type="text" id="currency" name="currency" value="{{ old('currency', $bill->currency) }}" maxlength="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">
                    </div>
                    <div>
                        <label for="bill_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Bill date') }} *</label>
                        <input type="date" id="bill_date" name="bill_date" value="{{ old('bill_date', $bill->bill_date?->toDateString()) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">
                    </div>
                    <div>
                        <label for="due_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Due date') }} *</label>
                        <input type="date" id="due_date" name="due_date" value="{{ old('due_date', $bill->due_date?->toDateString()) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">
                    </div>
                </div>
                <div class="mb-4">
                    <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Notes') }}</label>
                    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">{{ old('notes', $bill->notes) }}</textarea>
                </div>
                <div class="mb-4">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm" id="lines-table">
                        ...
                        <tbody id="lines-tbody">
                            @foreach(old('lines', $lines ?: [['description' => '', 'amount' => '']]) as $i => $line)
                            <tr class="line-row">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="inline-flex px-4 py-2 rounded-md bg-blue-600 text-white text-sm font-medium hover:bg-blue-700">{{ __('Update bill') }}</button>
                    <a href="{{ route('accounts-payable.bills.show', $bill->id) }}" class="inline-flex px-4 py-2 rounded-md bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addBtn.addEventListener('click', function() {
            ...
            tr.innerHTML = '<td class="px-4 py-2"><input type="text" name="lines[' + idx + '][description]" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm" required></td>' +
                '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][amount]" step="0.01" min="0" class="w-full text-right rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm" required></td>' +
                '<td class="px-4 py-2"><button type="button" class="remove-line text-red-600 hover:text-red-800 text-sm">{{ __("Remove") }}</button></td>';
            tbody.appendChild(tr);
            tr.querySelector('.remove-line').addEventListener('click', function() { removeRow(tr); });
        });
        function removeRow(tr) {
            if (tbody.querySelectorAll('.line-row').length > 1) tr.remove();
        }
