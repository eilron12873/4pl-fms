<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @can('accounts-receivable.manage')
                <a href="{{ route('accounts-receivable.payments.create') }}" class="inline-flex items-center px-4 py-2 rounded-md bg-blue-600 text-white text-sm font-medium hover:bg-blue-700">{{ __('Record payment') }}</a>
            @endcan
        </div>
    </x-slot>
    <div class="py-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 mb-4">
            <form method="GET" action="{{ route('accounts-receivable.payments.index') }}" class="flex flex-wrap gap-4 items-end">
                <div>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($payments as $pmt)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-gray-900 dark:text-gray-100">{{ $pmt->client ? $pmt->client->code.' — '.$pmt->client->display_name : '—' }}</td>
