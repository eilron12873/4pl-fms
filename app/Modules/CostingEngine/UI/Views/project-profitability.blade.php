<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Project Profitability') }}</h2>
    ...
    <div class="py-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 mb-4">
            <form method="GET" action="{{ route('costing-engine.project-profitability') }}" class="flex flex-wrap gap-4 items-end">
                <div>
                ...
                <a href="{{ route('costing-engine.export', ['report' => 'project', 'from_date' => $fromDate, 'to_date' => $toDate]) }}" class="inline-flex px-4 py-2 rounded-md bg-blue-600 text-white text-sm hover:bg-blue-700">{{ __('Export CSV') }}</a>
            </form>
            ...
            <div class="p-4 overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-300">{{ __('Project ID') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($rows as $r)
                            <tr>
                                <td class="px-4 py-2 font-mono text-gray-900 dark:text-gray-100">
                                    <a href="{{ route('costing-engine.details', ['dimension' => 'project_id', 'id' => $r['project_id'], 'from_date' => $fromDate, 'to_date' => $toDate]) }}" class="text-blue-600 dark:text-blue-400 hover:underline">{{ $r['project_id'] }}</a>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('No project profitability data. Post journal lines with project_id and revenue/expense accounts to see results.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
