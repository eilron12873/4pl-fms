<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Client Profitability') }}</h2>
    ...
    <div class="py-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 mb-4">
            <form method="GET" action="{{ route('costing-engine.client-profitability') }}" class="flex flex-wrap gap-4 items-end">
                <div>
            ...
            <div class="p-4 overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 text-right">
                                    @if($r['margin_pct'] !== null)
                                        <span class="{{ $r['margin_pct'] >= 0 ? '' : 'text-red-600 dark:text-red-400' }}">{{ number_format($r['margin_pct'], 1) }}%</span>
                                    @else
                                        —
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('No client profitability data for the selected period. Revenue comes from issued AR invoices; cost from posted journal lines with client dimension.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
